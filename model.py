...
def xDeepFM_MTL(input_feature_list, 
    ...
    with tf.device('/cpu:0'):
        ...
        for feature_info in embedding_feature_list:
            ...
            tensor = EmbeddingFeatureColumnLayer(name, fc_dict[name], dimension=dimension, features=features, l2_reg=l2_reg_embedding)(inputs)
    ...
    for feature_info in numeric_feature_list:
        ...
        tensor = Dense(embedding_size, use_bias=False, kernel_regularizer=l2(0.00001))(tensor)
        #tensor = Dense(embedding_size, use_bias=False)(tensor)
        numeric_tensors[name] = tensor

    linear_input_list = list(linear_embedding_tensors.values()) + list(numeric_tensors.values())
    ...
    #    exFM_logit = tf.keras.layers.Dense(1)(exFM_out)
    ...
    finish_logit_list = [linear_logit, finish_logit]
    like_logit_list = [linear_logit, like_logit]
